import { Scene as f, WebGLRenderer as g, sRGBEncoding as y, PerspectiveCamera as w, Mesh as M, MeshStandardMaterial as b, Group as p, BufferGeometry as x, BufferAttribute as A } from "three";
import { CSS3DRenderer as E } from "three/addons/renderers/CSS3DRenderer.js";
import { C as R } from "./controller-g2BzTGYF.js";
import { U as S } from "./ui--lfYp8Dx.js";
import "react";
...
class C {
  constructor({
    ...
    this.container = a, this.ui = new S({ uiLoading: t, uiScanning: o, uiError: n }), this.controller = new R({
      filterMinCF: e,
      filterBeta: h
    }), this.disableFaceMirror = l, this.scene = new f(), this.cssScene = new f(), this.renderer = new g({ antialias: !0, alpha: !0 }), this.cssRenderer = new E({ antialias: !0 }), this.renderer.outputEncoding = y, this.renderer.setPixelRatio(window.devicePixelRatio), this.camera = new w(), this.userDeviceId = s, this.environmentDeviceId = c, this.anchors = [], this.faceMeshes = [], this.latestEstimate = null, this.container.appendChild(this.renderer.domElement), this.container.appendChild(this.cssRenderer.domElement), this.shouldFaceUser = !0, window.addEventListener("resize", this._resize.bind(this));
  }
  async start() {
        ...
        if (e) {
          ...
          for (let r = 0; r < this.anchors.length; r++) {
            ...
            if (this.anchors[r].css) {
              ...
              this.anchors[r].group.matrix.set(...m);
            } else
  ...
  _resize() {
      ...
      return;
    {
      ...
      const { fov: v, aspect: i, near: u, far: m } = this.controller.getCameraParams();
      this.camera.fov = v, this.camera.aspect = i, this.camera.near = u, this.camera.far = m, this.camera.updateProjectionMatrix(), this.renderer.setSize(this.video.videoWidth, this.video.videoHeight), this.cssRenderer.setSize(this.video.videoWidth, this.video.videoHeight);
    }
    let h, s;
